// UndoItemDelCppComment.cpp: CUndoItemDelCppComment クラスのインプリメンテーション
//
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
CUndoItemDelCppComment::CUndoItemDelCppComment()
{
	m_bIsCanUndo = TRUE;
	m_bIsCanRedo = TRUE;
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemDelCppComment::Do()
{
}
	...
	)
{
	...
	if( lSelectStartLine == lSelectEndLine ) //１行選択
	{
		...
		if( pobjLine != NULL )
		{
			...
			c2ndChar = pobjLine->GetAt(lSelectStartOffset+1);
			if( !IsDBCSLeadByteEx(0,c1stChar) && c1stChar == '/' &&
				!IsDBCSLeadByteEx(0,c2ndChar) && c2ndChar == '/' &&
				(lSelectEndOffset-lSelectStartOffset) >= 2
			)
			{
				//ダブルスラッシュのＣ＋＋コメント削除
				pobjLine->Trim(lSelectStartOffset,lSelectStartOffset+2);
	...
	if( bIsEndTrim )
		m_lEndOffset = lSelectEndOffset - 2;
	else
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemDelCppComment::doUndo()
{
	///////////////////////////////
	//	処理実行
	///////////////////////////////
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemDelCppComment::doRedo()
{
	UCHAR	c1stChar=0;
	UCHAR	c2ndChar=0;
	...
	if( m_lSelectStartLine == m_lSelectEndLine ) //１行選択
	{
		...
		if( pobjLine != NULL )
		{
			...
			c2ndChar = pobjLine->GetAt(m_lSelectStartOffset+1);
			if( !IsDBCSLeadByteEx(0,c1stChar) && c1stChar == '/' &&
				!IsDBCSLeadByteEx(0,c2ndChar) && c2ndChar == '/' &&
				(m_lSelectEndOffset-m_lSelectStartOffset) >= 2
			)
			{
				//ダブルスラッシュのＣ＋＋コメント削除
				pobjLine->Trim(m_lSelectStartOffset,m_lSelectStartOffset+2);
				m_pobjEditorDoc->SetOneLine(m_lSelectStartLine,pobjLine);
			}
		}
	}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CUndoItemDelCppComment::IsCanUndo()
{
	return m_bIsCanUndo;
}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CUndoItemDelCppComment::IsCanRedo()
{
	return m_bIsCanRedo;
}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemDelCppComment::setCanUndo
	(	BOOL	bIsCanUndo
	)
{
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemDelCppComment::setCanRedo
	(	BOOL	bIsCanRedo
	)
{
