...
//        rectt.right-=m_iconX;
		  ...
		  leftStr=strText;
		//メニューＩＤに対応するショートカットキーを検索
		DWORD dwMenuID = lpDIS->itemID;
		CKeyBind* pobjKeyBind=NULL;
		if((pobjKeyBind = ((CEditorApp*)AfxGetApp())->FindKeyBind(dwMenuID)) != NULL)
		{
			rightStr = pobjKeyBind->GetKeyBindString();
			rectt.right-=m_iconX;
		}
//2001.02.04 Modify End

//1999.03.10 Add Start By S.Ekura （キー操作の記録開始/終了の文字列切替え）
		objMsg.LoadString(IDS_TOOL_OPE_STARTENDRECORD);
		if(leftStr.Find(objMsg) != -1) 
		{
			CMainFrame*	pobjMainFrame = (CMainFrame*)AfxGetMainWnd();
			if(pobjMainFrame->IsNowOperationRecording()==TRUE)
				leftStr.LoadString(IDS_TOOL_OPE_ENDRECORD);
			else
				leftStr.LoadString(IDS_TOOL_OPE_STARTRECORD);
		}
		objMsg.LoadString(IDS_INCREMENTALSEARCH);
		if(leftStr.Find(objMsg) != -1) 
		{
			CMainFrame*	pobjMainFrame = (CMainFrame*)AfxGetMainWnd();
			if(pobjMainFrame->IsNowIncrimentalSearching()==TRUE)
				leftStr.LoadString(IDS_INCREMENTALSEARCH_E);
			else
				leftStr.LoadString(IDS_INCREMENTALSEARCH_S);
		}
//1999.04.21 Add End By S.Ekura
