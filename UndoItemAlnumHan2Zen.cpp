// UndoItemAlnumHan2Zen.cpp: CUndoItemAlnumHan2Zen クラスのインプリメンテーション
//
//////////////////////////////////////////////////////////////////////
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
CUndoItemAlnumHan2Zen::CUndoItemAlnumHan2Zen()
{
	m_bIsCanUndo = TRUE;
	m_bIsCanRedo = TRUE;
	...
	if( lSelectStartLine == lSelectEndLine ) //１行選択
	{
		...
		if( pobjLine != NULL )
		{
			...
			for(long nCnt=lSelectStartOffset; nCnt<lEndOffsetAfter; )
			{
				...
				{
					if( isAlnum(pobjLine->GetAt(nCnt)) ) //半角英数字
					{
						//下へ
					}
					else
				...
				//半角英数字取得
				unsigned char cHanChar = pobjLine->GetAt(nCnt);
				//半角英数字を削除
				unsigned char c1stChar=0, c2ndChar=0;
				pobjLine->DeleteChar(nCnt,&c1stChar,&c2ndChar);
				//全角英数字変換＆挿入
				UINT nZenChar = objCodeConvManager.AlnumToZen(cHanChar);
				pobjLine->InsertChar(nCnt,(((nZenChar>>8)&0x00ff)),(nZenChar&0x00ff));
				nCnt+=2;
				//終了オフセットを更新
				lEndOffsetAfter++;
			}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemAlnumHan2Zen::doUndo()
{
	///////////////////////////////
	//	処理実行
	///////////////////////////////
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemAlnumHan2Zen::doRedo()
{
	CCodeConvManager	objCodeConvManager;
	long lEndOffsetAfter = 0;
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CUndoItemAlnumHan2Zen::IsCanUndo()
{
	return m_bIsCanUndo;
}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CUndoItemAlnumHan2Zen::IsCanRedo()
{
	return m_bIsCanRedo;
}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemAlnumHan2Zen::setCanUndo
	(	BOOL	bIsCanUndo
	)
{
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemAlnumHan2Zen::setCanRedo
	(	BOOL	bIsCanRedo
	)
{
