// UndoItemInsDay.cpp: CUndoItemInsDay クラスのインプリメンテーション
//
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
CUndoItemInsDay::CUndoItemInsDay()
{
	m_bIsCanUndo = TRUE;
	m_bIsCanRedo = TRUE;
	...
	)
{
	...
	if( pobjLine != NULL )
	{
		pobjLine->Insert(lCurrentOffset,(const unsigned char*)(LPCTSTR)objDayStr);
		m_pobjEditorDoc->SetOneLine(lCurrentLine,pobjLine);
	}
	else
	{
		pobjLine = new CLine((const unsigned char*)(LPCTSTR)objDayStr);
	...
	m_lSelectEndOffset   = lCurrentOffset + objDayStr.GetLength();
...
void CUndoItemInsDay::doRedo()
{
	...
	//日付文字列挿入
	char* pPtr = m_objDayStr.GetBuffer(1024);
	CLine* pobjLine = m_pobjEditorDoc->GetOneLine(m_lCaretLine);
	if( pobjLine != NULL )
	{
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CUndoItemInsDay::IsCanUndo()
{
	return m_bIsCanUndo;
}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CUndoItemInsDay::IsCanRedo()
{
	return m_bIsCanRedo;
}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemInsDay::setCanUndo
	(	BOOL	bIsCanUndo
	)
{
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemInsDay::setCanRedo
	(	BOOL	bIsCanRedo
	)
{
