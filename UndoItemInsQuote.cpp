// UndoItemInsQuote.cpp: CUndoItemInsQuote クラスのインプリメンテーション
//
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
CUndoItemInsQuote::CUndoItemInsQuote()
{
	m_bIsCanUndo = TRUE;
	m_bIsCanRedo = TRUE;
	...
	)
{
	...
	if( lSelectStartLine == lSelectEndLine ) //１行選択
	{
		...
		if( pobjLine != NULL )
		{
			pobjLine->InsertChar(lSelectStartOffset,CODE_QUOTE,0);
	...
	else //複数行選択
	{
		for(ULONG lCnt=0; lCnt<(ULONG)(lSelectEndLine-lSelectStartLine+1); lCnt++ )
		{
			...
			if( pobjLine != NULL )
			{
				...
				{
					pobjLine->InsertChar(0,CODE_QUOTE,0);
				}
...
void CUndoItemInsQuote::doRedo()
{
	...
	if( m_lSelectStartLine == m_lSelectEndLine ) //１行選択
	{
		...
		if( pobjLine != NULL )
		{
			pobjLine->InsertChar(m_lSelectStartOffset,CODE_QUOTE,0);
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CUndoItemInsQuote::IsCanUndo()
{
	return m_bIsCanUndo;
}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CUndoItemInsQuote::IsCanRedo()
{
	return m_bIsCanRedo;
}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemInsQuote::setCanUndo
	(	BOOL	bIsCanUndo
	)
{
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemInsQuote::setCanRedo
	(	BOOL	bIsCanRedo
	)
{
