// UndoItemCutSelectArea.h: CUndoItemCutSelectArea クラスのインターフェイス
//
//////////////////////////////////////////////////////////////////////

#if !defined(UNDOITEMCUTSELECTAREA_H)		//２重インクルード防止
#define UNDOITEMCUTSELECTAREA_H
...
class CUndoItemCutSelectArea : public CUndoItem
{
public:
	CUndoItemCutSelectArea();								// コンストラクタ
	CUndoItemCutSelectArea(CEditorDoc*);					// コンストラクタ
	virtual ~CUndoItemCutSelectArea();						// デストラクタ

	void	Do();											// 処理実行
	void	Do(long,long,long,long,BOOL,CPoint&,CPoint&,BOOL,BOOL);
	void	doUndo();										// Undo 実行
	void	doRedo();										// Redo 実行
...
private:
	...
	BOOL		m_bToClipBoard;								// クリップボードへコピーするか？（TRUE:する、FALSE:しない）
...
#endif // !defined(UNDOITEMCUTSELECTAREA_H)
