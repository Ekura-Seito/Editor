// UndoItemKatakanaHan2Zen.cpp: CUndoItemKatakanaHan2Zen クラスのインプリメンテーション
//
	...
	if( lSelectStartLine == lSelectEndLine ) //１行選択
	{
		...
		if( pobjLine != NULL )
		{
			...
			for(long nCnt=lSelectStartOffset; nCnt<lEndOffsetAfter; )
			{
				...
				{
					if( isHanKana(pobjLine->GetAt(nCnt)) ) //半角カタカナ
					{
						//下へ
					}
					else
				...
				if( !IsDBCSLeadByteEx(0,pobjLine->GetAt(nCnt+1)) && // 次が ﾞ
				...
				else if( !IsDBCSLeadByteEx(0,pobjLine->GetAt(nCnt+1)) && // 次が ﾟ
