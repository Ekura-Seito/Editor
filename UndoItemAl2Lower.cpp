// UndoItemAl2Lower.cpp: CUndoItemAl2Lower クラスのインプリメンテーション
//
//////////////////////////////////////////////////////////////////////
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
CUndoItemAl2Lower::CUndoItemAl2Lower()
{
	m_bIsCanUndo = TRUE;
	m_bIsCanRedo = TRUE;
	...
	if( lSelectStartLine == lSelectEndLine ) //１行選択
	{
		...
		if( pobjLine != NULL )
		{
			for(long nCnt=lSelectStartOffset; nCnt<lSelectEndOffset; )
			{
				...
				{
					if( isAlBig(pobjLine->GetAt(nCnt)) ) //英大文字
					{
						//下へ
					}
					else
				...
				//英大文字取得
				UINT nBigChar = pobjLine->GetAt(nCnt);
				//英大文字を削除
				unsigned char c1stChar=0, c2ndChar=0;
				pobjLine->DeleteChar(nCnt,&c1stChar,&c2ndChar);
				//小文字変換＆挿入
				UINT nSmallChar = nBigChar + 0x20;
				pobjLine->InsertChar(nCnt,nSmallChar,0);
				nCnt++;
			}
		...
		for(ULONG lLineCnt=0; lLineCnt<(ULONG)(lSelectEndLine-lSelectStartLine+1); lLineCnt++ )//行ループ
		{
			...
			if( lLineCnt == 0 ) //選択開始行
			{
				for(long nCnt=lSelectStartOffset; nCnt<pobjLine->GetLength(); )
				{
					...
					//英大文字取得
					char cBigChar = pobjLine->GetAt(nCnt);
					//英大文字を削除
					unsigned char c1stChar=0, c2ndChar=0;
					pobjLine->DeleteChar(nCnt,&c1stChar,&c2ndChar);
					//小文字変換＆挿入
					char cSmallChar = cBigChar + 0x20;
					pobjLine->InsertChar(nCnt,cSmallChar,0);
					nCnt++;
				}
			}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CUndoItemAl2Lower::IsCanUndo()
{
	return m_bIsCanUndo;
}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CUndoItemAl2Lower::IsCanRedo()
{
	return m_bIsCanRedo;
}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemAl2Lower::setCanUndo
	(	BOOL	bIsCanUndo
	)
{
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CUndoItemAl2Lower::setCanRedo
	(	BOOL	bIsCanRedo
	)
{
