// DecriptDlg.cpp : インプリメンテーション ファイル
//
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
CDecriptDlg::CDecriptDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CDecriptDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDecriptDlg)
	...
	m_objKey = _T("");						// 復号化キー
	m_objFolder = _T("");					// 復号化実行フォルダ
	m_objFileKind = _T("");					// 復号化対象のファイル種別
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CDecriptDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDecriptDlg)
...
BEGIN_MESSAGE_MAP(CDecriptDlg, CDialog)
	//{{AFX_MSG_MAP(CDecriptDlg)
...
BOOL CDecriptDlg::OnInitDialog() 
{
	...
	objMsg.LoadString(IDS_DECRYPTKEY);
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CDecriptDlg::OnSize(UINT nType, int cx, int cy) 
{
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CDecriptDlg::PreTranslateMessage(MSG* pMsg)
{
...
void CDecriptDlg::OnOK() 
{
	...
	if(m_objKey.IsEmpty())
	{
		objMsg.LoadString(IDS_INPUT_DECRIPTKEY);
	...
	{
		objMsg.LoadString(IDS_LESS16_DECRIPTKEY);
		m_pApp->MyMessageBox(objMsg,(MB_OK|MB_ICONSTOP),0);
		return;
	}
	if( (m_objKey.GetLength() % 2) != 0 )
	{
		objMsg.LoadString(IDS_ODD_DECRIPTKEY);
		m_pApp->MyMessageBox(objMsg,(MB_OK|MB_ICONSTOP),0);
		return;
	}
	m_objKey.MakeUpper();
	for(long lCnt=0; lCnt<m_objKey.GetLength(); lCnt++)
	{
		if(IsDBCSLeadByteEx(0,m_objKey.GetAt(lCnt)))//２ﾊﾞｲﾄ文字の第一ﾊﾞｲﾄ
		{
			objMsg.LoadString(IDS_ASCIONLY_DECRIPTKEY);
			m_pApp->MyMessageBox(objMsg,(MB_OK|MB_ICONSTOP),0);
			return;
		}
		...
		{
			objMsg.LoadString(IDS_ASCIONLY_DECRIPTKEY2);
			m_pApp->MyMessageBox(objMsg,(MB_OK|MB_ICONSTOP),0);
			return;
		}
	}
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
BOOL CDecriptDlg::LoadWindowPlacement(LPWINDOWPLACEMENT pwp)
{
	CString strBuffer = m_pApp->GetProfileString("CDecriptDlg","WindowPos");
...
/*|	I/F		：																		|*/
/*[0]-----------------------------------------------------------------------------[0]*/
void CDecriptDlg::SaveWindowPlacement(LPWINDOWPLACEMENT pwp)
{
	//ウインドウの位置を保存する
	CString	strBuffer;
	strBuffer.Format("%i:%i:%i:%i:%i:%i:%i:%i:%i:%i",
					...
					pwp->rcNormalPosition.left,  pwp->rcNormalPosition.top,
					pwp->rcNormalPosition.right, pwp->rcNormalPosition.bottom);
	m_pApp->WriteProfileString("CDecriptDlg","WindowPos",strBuffer);
}
